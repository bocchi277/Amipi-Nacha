    ...
    users.forEach((u, idx) => {
      ...
      if (!isSelf) {
        toggleBtn = u.is_active
          ? `<button type="button" class="btn btn-danger btn-sm" onclick="AdminScreen.toggleUserStatus('${u.id}', '${u.username}', true)" style="font-size: var(--text-xs); padding: 2px 8px;">Deactivate</button>`
          : `<button type="button" class="btn btn-success btn-sm" onclick="AdminScreen.toggleUserStatus('${u.id}', '${u.username}', false)" style="font-size: var(--text-xs); padding: 2px 8px;">Activate</button>`;
      } else {
      ...
      tr.innerHTML = `
        ...
        <td style="text-align: right;">
          <div style="display: flex; gap: var(--space-xs); justify-content: flex-end; align-items: center;">
            <button type="button" class="btn btn-secondary btn-sm" onclick="AdminScreen.openResetPasswordModal('${u.id}', '${u.username}')" style="font-size: var(--text-xs); padding: 2px 8px;">
              Reset Password
            </button>
            ${toggleBtn}
          </div>
