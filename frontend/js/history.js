...
const HistoryScreen = (() => {
  ...
  function bindEvents() {
    ...
    const filterInputIds = [
      ...
      'colFilterAmount', 'colFilterInvoice', 'colFilterSequence',
      'colFilterGeneratedBy', 'colFilterSentAt',
    ];
  ...
  function applyColumnFilters() {
    ...
    const sequence = (el('colFilterSequence') ? el('colFilterSequence').value : '').trim().toLowerCase();
    ...
    filteredRemittances = allRemittances.filter(r => {
      ...
      if (sequence && !(r.sequence_id || '').toLowerCase().includes(sequence)) return false;
  ...
  function clearColumnFilters() {
    ['colFilterEffDate', 'colFilterVendor', 'colFilterEmail',
     'colFilterAmount', 'colFilterInvoice', 'colFilterSequence',
     'colFilterGeneratedBy', 'colFilterSentAt', 'filterMonthFrom',
     'filterMonthTo', 'filterAmountMin', 'filterAmountMax'].forEach(id => {
      const input = el(id);
  ...
  function handleExportCSV() {
    ...
    const headers = ['ID', 'Effective Date', 'Vendor Name', 'Recipient Email', 'Amount ($)', 'Invoice / Ref #', 'Sequence ID', 'Status', 'Sent At'];
  ...
  function handleExportExcel() {
    ...
    const headers = ['#', 'Effective Date', 'Vendor Name', 'Recipient Email', 'Amount ($)', 'Invoice / Ref #', 'Sequence ID', 'Generated By', 'Status', 'Sent At'];
  ...
  function renderTable() {
    ...
    pageItems.forEach((r, i) => {
      ...
      tr.innerHTML = `
        ...
        <td class="font-mono" style="text-align: center; color: #64748B; font-size: 11px;">${globalIdx}</td>
        <td class="font-mono text-xs" style="text-align: center; color: #334155; font-size: 11px;">${formattedEffDate}</td>
        <td class="font-bold" style="color: #0F172A; font-size: 12px; max-width: 140px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;" title="${r.vendor_name}">${r.vendor_name}</td>
        <td class="font-mono text-xs text-muted" id="email-cell-${r.id}" style="max-width: 155px;">
          <div id="email-display-${r.id}" style="display: inline-flex; align-items: center; justify-content: space-between; width: 100%; gap: 4px;">
            <span style="overflow: hidden; text-overflow: ellipsis; white-space: nowrap; max-width: 125px;" title="${r.recipient_email}">${r.recipient_email}</span>
            <button type="button" class="btn btn-secondary btn-sm" onclick="HistoryScreen.editRowEmail('${r.id}')" style="padding: 1px 4px; font-size: 9px; line-height: 1; border-radius: 3px; flex-shrink: 0;" title="Edit Recipient Email Address">
              <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-pencil"><path d="M21.174 6.812a1 1 0 0 0-3.986-3.987L3.842 16.174a2 2 0 0 0-.5.83l-1.321 4.352a.5.5 0 0 0 .623.622l4.353-1.32a2 2 0 0 0 .83-.497z"/><path d="m15 5 4 4"/></svg>
            </button>
          </div>
          <div id="email-edit-${r.id}" style="display: none; align-items: center; gap: 3px; width: 100%;">
            <input type="email" class="form-input form-input-sm" id="email-input-${r.id}" value="${r.recipient_email}" style="padding: 2px 4px; font-size: 10px; width: 105px;" onkeydown="if(event.key==='Enter'){HistoryScreen.saveRowEmail('${r.id}');}if(event.key==='Escape'){HistoryScreen.cancelRowEmailEdit('${r.id}');}" />
            <button type="button" class="btn btn-primary btn-sm" onclick="HistoryScreen.saveRowEmail('${r.id}')" style="padding: 2px 4px; font-size: 9px;" title="Save Email">✓</button>
            <button type="button" class="btn btn-secondary btn-sm" onclick="HistoryScreen.cancelRowEmailEdit('${r.id}')" style="padding: 2px 4px; font-size: 9px;" title="Cancel">✕</button>
          </div>
        </td>
        <td class="font-mono font-bold" style="text-align: right; color: #0F172A; font-size: 12px; white-space: nowrap;">${amtFormatted} ${breakdownBadge}</td>
        <td class="font-mono text-xs" style="color: #334155; font-size: 11px; max-width: 110px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;" title="${r.invoice_reference || ''}">${r.invoice_reference || '—'}</td>
        <td class="font-mono text-xs" style="text-align: center; white-space: nowrap;">${r.sequence_id ? `<span class="badge" title="Full Trace: ${r.sequence_id}" style="background: #F8FAFC; border: 1px solid #CBD5E1; color: #1E293B; font-size: 10.5px; padding: 2px 6px; font-family: monospace; letter-spacing: 0.5px; border-radius: 4px;">${r.sequence_id.length > 6 ? r.sequence_id.slice(-6) : r.sequence_id}</span>` : '<span class="text-muted">—</span>'}</td>
        <td class="font-mono text-xs" style="text-align: center; max-width: 85px;"><span class="badge badge-secondary" title="${r.created_by_username || 'admin'}" style="background: #F1F5F9; color: #475569; border: 1px solid #E2E8F0; padding: 2px 6px; border-radius: 4px; max-width: 75px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; display: inline-block;">${r.created_by_username || 'admin'}</span></td>
        <td style="text-align: center;">${statusBadge}</td>
        <td class="font-mono text-xs text-muted" style="text-align: center; font-size: 10.5px; white-space: nowrap;">${formattedSentDate}</td>
        <td style="text-align: center; white-space: nowrap;">
          <div class="history-action-group">
            <button type="button" class="btn btn-secondary btn-sm" onclick="HistoryScreen.openViewEmailModal('${r.id}')" style="padding: 2px 5px; font-size: 10px; display: inline-flex; align-items: center; gap: 2px; border-radius: 4px;" title="View Rendered Tabular Email">
            ...
            <button type="button" class="btn btn-secondary btn-sm" onclick="HistoryScreen.sendSingleRemittanceEmail('${r.id}')" style="padding: 2px 5px; font-size: 10px; display: inline-flex; align-items: center; gap: 2px; border-radius: 4px;" title="Send Remittance Email to ${r.recipient_email}">
            ...
            ${isAdmin() ? `
            <button type="button" class="btn btn-sm" onclick="HistoryScreen.openConfirmDeleteSingleHistory('${r.id}')" style="background: #FEF2F2; color: #DC2626; border: 1px solid #FECACA; padding: 2px 5px; font-weight: 600; font-size: 10px; display: inline-flex; align-items: center; gap: 2px; border-radius: 4px;" title="Delete Record (Admin Only)">
